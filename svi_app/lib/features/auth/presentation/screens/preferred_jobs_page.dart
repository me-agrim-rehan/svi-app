...
class PreferredJobsPage extends StatefulWidget {
  ...
  // ---- CONTROLLED MODE (used embedded in registration's PageView) ----
  // Parent (register_screen.dart) owns the selection list and passes it in,
  // along with a callback to toggle it. No Save button shown — registration
  // saves everything together at the end via createAccount().
  final List<String>? selectedJobs;
  final void Function(String jobKey)? onToggle;

  // ---- STANDALONE MODE (used from Profile page) ----
  // This page owns its own selection state, shows a back button + Save
  // button, and persists directly via ProfileService when you push it with
  // standalone: true. `selectedJobs` is used as the starting selection.
  final String? phone;
    ...
    return SingleChildScrollView(
      ...
      child: Column(
        ...
        children: [
          ...
          Text(
            'Pick up to ${widget.maxSelections} (${selected.length}/${widget.maxSelections} selected)',
                ...
                children: [
                  ...
                  Wrap(
                    ...
                    children: entry.value.map((subrole) {
                      ...
                      final isSelected = selected.contains(jobKey);
                      final isDisabled = !isSelected && _limitReached;

                      return ChoiceChip(
                        label: Text(subrole),
                        selected: isSelected,
                        onSelected: isDisabled ? null : (_) => _handleToggle(jobKey),
